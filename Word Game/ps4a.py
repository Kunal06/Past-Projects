...
def getWordScore(word, n):
    ...
    for letter in range(len(word)):
        # add up the scores according to Scabble values
        score += SCRABBLE_LETTER_VALUES[word[letter]]

    #multiply score by length of word
    score *= len(word)

    # add 50 bonus points if all letters used
    if (n == len(word)):
        score += 50
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while(True):
        ...
        while(userResponse != '.'):
            displayHand(hand)
            userResponse = raw_input("Enter word, or a \".\" to indicate that you are finished: ")

            if (userResponse == '.'):
                break
            
            # If the word is not valid:
            if not isValidWord(userResponse, hand, wordList):
            ...
            # Otherwise (the word is valid):
                ...
                wordScore = getWordScore(userResponse, n)
                totalScore += wordScore
                print ("\"" + userResponse + "\" earned " + str(wordScore) + " points. Total: " + str(totalScore) + " points.")
                print('')
                # Update the hand 
                hand = updateHand(hand, userResponse)
